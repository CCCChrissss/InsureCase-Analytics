...
- `backend/tests/test_api.py`：API smoke tests，覆蓋 health、statistics、cases、case detail、完整原文結構化與字元涵蓋、PDF、search、summary、similar、semantic-similar、semantic model params 與 quality。
- `backend/tests/test_document_section_service.py`：原文結構化單元測試，驗證逐字還原、相對人主張與非標準格式 fallback。
...
- `frontend/src/api/client.ts`：API base URL 與共用 `apiGet`。
...
- `frontend/src/components/CaseDetailView.tsx`：理賠人員導向的完整案件結構化區塊、評議結果、法源、相關案件及逐字全文閱讀區；主文與爭點預設展開，其他區塊可個別或全部展開。
...
- `frontend/src/pages/CalculationMethodPage.tsx`：公開 chunking、FTS5/BM25、query-to-case、相關案件、百分比、評議結果與 Precision@5 的公式、流程及限制。
- `frontend/src/pages/`：案件工作台、全文搜尋、計算方法、語意搜尋與分析驗證頁；前三者為主要使用流程，技術驗證頁保留 direct route。
- `frontend/src/utils/legalReferences.ts`：使用法規名稱白名單擷取法規條號，刻意排除保單、附約與契約條款。
- `frontend/src/utils/caseText.ts`：選擇完整 normalized/raw 案件文字、fallback 揭露與 Unicode 字數完整性核對，不做內容切片。
- `frontend/tests/legalReferences.test.ts`：以 Node 內建測試確認法規保留且契約條款不進入法源依據。
- `frontend/tests/caseText.test.ts`：確認長案件文字的開頭、中段、結尾均保留，並驗證文字來源 fallback 與字數核對。
...
- 全文搜尋：接受關鍵字、短句或最長 2,000 字元的事件經過；預設會對全部 17,254 個 chunks 執行語意召回，再與 FTS5 / LIKE 文字排名融合。畫面將「查找範圍」與「排序方向」拆成獨立選單，可顯示全部案件或只保留文字命中，並可將完整排名由高到低或低到高排列後再分頁。模型失敗時後端自動退回文字搜尋。
...
```text
GET /api/cases/{case_id}/document-sections
```

用途：以 normalized text 優先、raw text fallback，回傳案件完整原文的有序結構化區塊。每段包含類型、標題、原始 heading、完整內容、字數與起訖 offset；`source_chars`、`covered_chars` 與 `complete_coverage` 用於確認切分沒有漏字。若原始格式缺少標準標題，內容會併入相鄰段落或 `other`，不會被丟棄。
...
- `result_scope`：`all` 或 `keyword`，預設 `all`；決定是否只保留有文字命中的案件。
- `sort_direction`：`desc` 或 `asc`，預設 `desc`；決定完整綜合排名方向。
...
5. 合併後依 `result_scope` 篩選、依 `sort_direction` 決定方向，最後才分頁；DB 身分、query、provider 與 model 相同時最多快取最近 16 組完整排名，切換範圍或方向不重跑模型。
...
用途：將全部關鍵字命中案件依 query-to-case 語意相似度全域排序，完成後才套用分頁。
...
1. 使用與 `/api/search` 相同的 FTS5 / LIKE fallback 取得全部命中案件。
2. 將 query 轉為正規化向量，逐案計算所有 chunks 的 cosine similarity。
3. 每案取最高 chunk 分數，再依分數、日期與案號排序。
4. 全域排序完成後才切出指定頁面；缺少對應 embedding 的案件保留在結果尾端。
5. 以 DB 檔案身分、query、provider 與 model 組成快取 key，程序內最多保留最近 16 組完整排名。

回傳 `similarity_score`、最高分 chunk 的 `section_hint`、`chunk_index`、`semantic_snippet`、`cached` 與全文搜尋 `match_source`。畫面百分比為 `round(clamp(score, 0, 1) * 100)`。
...
- 後端 pytest 測試。
- 案件詳情展示鏈 API 測試，覆蓋詳情、結構化原文、PDF、摘要、規則式相似案件與語意相似案件。
...
- API 錯誤回應格式統一。
- 正式 React Router。
- 完整前端元件與端對端自動化測試；目前有法源過濾與完整文字選擇的 Node 單元測試、production build 與人工瀏覽器驗證。
- 規則式摘要與法源名稱擷取仍可能漏抓；正式理賠判斷必須回查原文、PDF 與現行法規，不可把前端整理內容當成法律結論。
...
- 全域語意排序、排序後分頁、第二頁快取與 API schema tests。
...
Invoke-WebRequest -Uri "http://127.0.0.1:8000/api/semantic-ranked-search?q=癌症&page=1&page_size=10&embedding_provider=local_bge&embedding_model=BAAI%2Fbge-large-zh-v1.5-local" -UseBasicParsing
...
- `/api/semantic-ranked-search?q=癌症` 第一頁最後一筆分數應大於或等於第二頁第一筆，且第二次翻頁請求的 `cached` 應為 `true`。
...
- 案件詳情可看到案件摘要。
- 案件詳情可看到相似案件。
- 搜尋頁可查「癌症」，將查找範圍切成只看文字命中後仍有 62 件；排序方向可獨立切換高到低或低到高，並維持 10／15／20 筆分頁。
- 計算方法頁可查看 BM25、搜尋相似度、Dashboard 相關案件、百分比、評議結果與 Precision@5 說明。
- 案件 Dashboard 的法源依據只顯示法規條文，不顯示保單、附約或契約條款。
...
2026-08-05 搜尋與計算說明功能完成後，另完成以下檢查：

- `.\.venv\Scripts\python.exe -m pytest -q`：121 passed。
- `node --test tests/legalReferences.test.ts`：1 passed，確認法源依據會排除保單、附約與契約條款。
- `pnpm build`：TypeScript 與 Vite production build 通過。
- `verify_case_db.py --expected-count 2992 --require-chunks --require-embeddings`：passed；2992 案、17254 chunks、17254 embeddings，案件路徑錯誤 0。
- 本機 BGE 全域排序實測：「癌症」共 62 件，先完成全部命中案件計分，再依相似度排序及分頁；第 2 頁使用查詢快取。
- 瀏覽器人工驗證：相似度排序由高到低、計算方法頁公式、法源依據法規白名單及 console 無 error 均通過。

2026-08-06 完整案件結構化修正後，完成以下檢查：

- 根因確認：`case_summaries` 的申請人主張與判斷理由是規則式節錄，且沒有相對人主張欄位，因此不適合作為案件 Dashboard 的完整內容來源。
- 新增 `/api/cases/{case_id}/document-sections`，直接由完整 normalized/raw text 切分；所有區塊是連續 source slice，可逐字串接還原原文。
- 全資料 2,992 件逐案驗證：字元涵蓋失敗 0 件，2,992 件均辨識到相對人主張；標題變形不影響內容保留。
- 最長案件 `113年評字第003819號`：SQLite 與案件詳情 API 的 normalized text 均為 23,669 字，開頭、中段與結尾逐字一致。
- `py -m pytest -q`：128 passed；`node --test tests/caseText.test.ts tests/legalReferences.test.ts`：3 passed。
...
2026-08-06 第一案摘要 v6 顯示清理後，另完成以下檢查：

- 摘要顯示會依原文區塊類型移除 `主文`、`四、兩造不爭執之事實`、`(二)陳述`、`五、本件爭點` 與 `八、據上論結` 等結構前綴；evidence payload 仍保留逐字原文。
- 第一案以本機 `qwen3:4b` 重跑 9 次 request，失敗 0、final fallback 0，prompt version 為 `local_llm_summary_v6`。
- v6 validator：1 案、8 段 evidence、2 筆法源、`violations = []`；經使用者確認後已標記 approved，Dashboard 實測顯示「已人工確認」，API 為 official true。
- Trial DB 共 7 筆版本，狀態為 approved 1、rejected 1、unreviewed 5，`integrity_check = ok`；正式 DB 未修改。
- `py -m pytest -q`：159 passed，新增章節前綴清理、實際 disposition 分類與 evidence 不變 regression tests。

2026-08-10 其餘四案摘要品質修正後，另完成以下檢查：

- 四案最新版本為 `local_llm_summary_v7`、`v8`、`v9`、`v11`；各案由本機 `qwen3:4b` 產生，request error 皆為 0，未使用 Hugging Face Inference API。
...
2026-08-17 主搜尋拆分查找範圍與排序方向後，另完成以下檢查：

- `result_scope=all|keyword` 與 `sort_direction=desc|asc` 會在完整綜合排名上先篩選、反轉，再執行分頁；切換兩個控制不會重跑 Local BGE。
- 自然敘述高到低前三筆的 cosine 顯示皆約 70%；低到高第一頁由約 50%、51%、52% 開始，確認不是只反轉目前頁面。
- 「癌症」搭配只看文字命中仍為 62 件、每頁 15 筆、共 5 頁；高到低第一筆約 58%，切換方向會沿用快取。
- 後端完整測試為 198 passed；前端 Node 測試 3 passed；前端 production build、Python `py_compile` 與瀏覽器實際操作均通過，console 無 warning 或 error。
...
- 已將理賠人員主搜尋改為全庫語意召回與 FTS5 / LIKE 文字排名融合，每頁可選擇 10、15 或 20 筆；一般查找不要求先有關鍵字命中。查找範圍與排序方向為獨立控制，可只保留文字命中並將完整排名由高到低或低到高排列，bounded LRU cache 可在切換控制與翻頁時沿用。語意服務失敗時會退回關鍵字排序。
