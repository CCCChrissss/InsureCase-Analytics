...
7. 模型改寫無法逐字核對時，顯示已驗證原文引文；短區塊漏抓時才使用可標示的原文補位。
8. 評議理由另從完整理由區塊擷取含 `從而`、`足認`、`難認`、`有據` 等訊號的完整句，按本案套用訊號排序。
9. 法源同時接受模型驗證與規則擷取，但只保留原文中的正式法規名稱與條號；保單、附約與契約條款不列為法源。
10. 最終 JSON 保留 model、prompt version、source hash、原文 offset、拒絕／補位／失敗數與本機推論耗時。
