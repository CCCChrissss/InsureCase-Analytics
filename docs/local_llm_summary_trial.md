...
11. 模型改寫無法逐字核對時，顯示已驗證原文引文；短區塊漏抓時才使用可標示的原文補位。
12. 評議理由另從完整理由區塊擷取含 `從而`、`足認`、`難認`、`有據` 等訊號的完整句，按本案套用訊號排序。
13. 法源同時接受模型驗證與規則擷取，但只保留評議方採用的正式法規名稱與條號；當事人自行引用的法條、保單、附約與契約條款不列為法源。
14. 最終 JSON 保留 model、prompt version、source hash、原文 offset、拒絕／補位／失敗數與本機推論耗時。
...
- 五案已匯入 Trial DB 的獨立版本表並接入案件 Dashboard；第一案 v6 已依人工驗收標記為 `approved`，其餘四案的最新可用版本仍為 `unreviewed`。
...
- Trial DB 目前共 11 筆版本：`approved = 1`、`rejected = 1`、`unreviewed = 9`。四案最新版本已匯入但未自動核准；舊 v4 與第一案 v5 暫留作版本稽核。
